...
# assign directory
directory = 'results/CPLEXvsVNS/VNS_random' 

indicator = False
column_start = 0

workbook = xlsxwriter.Workbook('results/CPLEXvsVNS/composeVNS.xlsx')
        ...
        if k.endswith('txt'):
            ...
            with open(filename.path, 'r') as f:
                for line in f:
                    ...
                    if line[0].split('.')[0]+"."+line[0].split('.')[1] in kdom[k].keys():
                        kdom[k][line[0].split('.')[0]+"."+line[0].split('.')[1]].append([int(float(line[1]))]+line[2:])
                    else:
                        kdom[k][line[0].split('.')[0]+"."+line[0].split('.')[1]] = [[int(float(line[1]))]+line[2:]]
...
for k, v in kdom.items():
    ...
    for kp, vp in v.items():
        ...
        for l in vp:
            ...
            time_total.append(float(l[4]))
        
        kdom_new[k].append([kp, min(len_res), mean(len_res), stdev(len_res), mean(time_best_res), mean(time_total), all(sol_true)])
...
worksheet.write(row, 4, "mean_best_time")
worksheet.write(row, 5, "mean_total_time")
worksheet.write(row, 6, "all_feasible")
row +=1
