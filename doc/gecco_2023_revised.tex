...
MkDP has applications in distributed systems ~\cite{wang2013minimising}, where a $k$-dominating set in these systems is a set of processors such that each processor outside \textcolor{red}{this set} must have at least $k$ neighbors in the set.
...
($i$) \textsc{Vns} for MkDPA significantly improves the state-of-the-art results for all considered problem instances, where $k \in \{1, 2, 4\}$; ($ii$) \textsc{Vns} is able almost always quickly produce solutions of reasonable quality which is not the case for approaches previously proposed in the literature.
