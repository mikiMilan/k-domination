...
%% article.
\begin{abstract}
  In this paper, we deal with solving a generalized version of the well--known minimum dominating set problem, the so-called $k$--domination problem, $k \in \mathbb{N}$. This problem asks for finding a minimum cardinality subset $D$ of vertices of a graph  $G=(V, E) $ so that each $v \in V$ belongs to $D$ or has at least $k$ neighbours from $D$. The $k$-domination problem has an application in distributed systems where a set of processors has to be chosen satisfying that each processor outside the set has at least $k$ neighbours in the set; another application includes analysis of biological networks. For solving the $k$--domination problem, a variable neighbourhood search (VNS) meta-heuristic is proposed. VNS is equipped with a non-linear, efficiently evaluated fitness function that allows the VNS to search through feasible regions but also unfeasible regions, penalizing those solutions. The parameters of our VNS are tuned by two tuning tools; one is based on the evolutionary strategy and the second one on the grid--search strategy. The two obtained settings of VNS are compared to the best-known heuristic approaches from the literature: a beam search and the two greedy approaches. Experimental evaluations are conducted on the real-world benchmark set whose instances represent the street networks of respective cities. Our VNS  produce new state-of-the-art results on the most of the considered small and large--sized problem instances,
