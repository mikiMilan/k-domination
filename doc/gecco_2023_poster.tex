...
In this section we first give an overview over the variable neighborhood search (VNS) scheme. Then, the main details on the VNS to solve MkDP are provided: the fitness evaluation, shaking procedure and an efficient local search.
 ...
 VNS takes for input a solution $s_{best}$, which may be obtained as the outcome of a greedy or some randomized procedure, $d_{min}, d_{max} \in \mathbf{N}$, 
  ...
  \begin{itemize}
   ...
   Note that this function evaluates both, feasible and unfeasible solutions. 
       ...
       \begin{itemize}
       	...
       	\item Between two unfeasible solutions of the same cardinalty, one whose \emph{violate}($\cdot$) score is smaller is preferred, thus the solution is intuitively easier to be reshaped into a feasible one.  
  ...
  The purpose of local search (LS) is improving a solution $s'$ obtained in the shaking phase by applying a larger number of small changes on  the solution's structure. Therefore, defining  the ``small changes'' which reflects on the term LS neighboring relation, plays a crucial role in establishing an effective LS.  For MkDP, a pseudocode of the LS is given in Algorithm~\ref{alg:ls}.
