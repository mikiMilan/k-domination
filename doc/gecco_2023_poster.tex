...
In this section we first give an overview over the variable neighborhood search (VNS) scheme. Then, the main details on the VNS to solve MkDP are provided: the fitness evaluation, shaking procedure and an efficient local search.
 ...
 VNS takes for input a solution $s_{best}$, which may be obtained as the outcome of a greedy or some randomized procedure, $d_{min}, d_{max} \in \mathbf{N}$, 
  ...
  \begin{itemize}
  	...
  	\item \texttt{AcceptanceCriterion}($s_{best}, s', fitness(\cdot)$): this function provided a condition when to accept a solution $s'$ for as new incumbent, that is $s_{best} = s'$; in that case, $d$ is set back to $\min(d_{min}, |s|/2)$ if $|s|>2$; otherwise $d$ is incrementally increased. 
...
\subsection{Parameters tuning}
   ...
   \fxnote{Milan: dio oko podesavanja tuning alata -- koje instance, koliki je budet broja pustanja, isl.}
     ...
     We emphasize that tuning of all (five) large-sized instances and each $k \in \{1,2,4\}$  by means of \texttt{PyGAD} tool using the default configuration parameters has not been efficient. Thus, we determined a reasonable  value of the parameters  greedily, by testing a few carefully chosen parameters configurations. Among them, the best performing configuration w.r.t. average solution quality has the following values: $d_{min}=1, d_{max} = + \infty, penalty = 0.01, prob=0.5$. 

\subsection{Numerical results }
 Numerical results are reported in the three Tables~\ref{tab:k1}--\ref{tab:k4}, one per each $k \in \{1, 2,4\}$. The first column gives the instance name for which the results are reported; there are 20 small-sized instances, and 5 large-sized instance (at the very bottom). The next two blocks provide the results of \textsc{Vns} (the best result within ten runs and the average solution quality) and the best approach from the literature on the respective instance (the  name of the algorithm achieving the best result, its best result within ten runs, and its average solution quality). 
 ...
 \fxnote{Marko: stigao dovde!}
  ...
  \begin{table}
	...
	Belgrade&\bf{85}&88.7&PG&99&100.3\\
	Berlin&\bf{102}&104.7&SG&146&147\\
	Boston&99&101.1&SG&\bf{71}&72.6\\
	Dublin&96&99.7&PG&\bf{88}&90.2\\
	Minsk&\bf{100}&102.2&SG&138&139.3\\
	\hline
...
\begin{table}
	...
	\begin{tabular}{l|rrr|lrr}
		...
		Belgrade&\bf{169}&172.3&PG&197&197.5\\
		Berlin&\bf{206}&207.1&PG&268&270.4\\
		Boston&181&194.6&PG&\bf{133}&135\\
		Dublin&181&184.4&PG&\bf{166}&166.8\\
...
\begin{table}
	...
	\begin{tabular}{l|rrr|lrr}
		...
		Belgrade&\bf{344}&346.3&SG&397&400.1\\
		Berlin&\bf{408}&409.2&PG&501&503.4\\
		Boston&341&341&PG&\bf{255}&255.4\\
