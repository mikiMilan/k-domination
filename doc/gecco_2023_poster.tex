  ...
  The basic VNS scheme for solving the MkDP is given in Algorithm~\ref{alg:vns}.
     ...
     \begin{algorithm}[!t] 
  	...
  	\begin{algorithmic}[1]
  	    ...
  	    \STATE $d \gets d_{\min}$
  	    \IF{$|s|> 4$}
  	 	    \STATE  $d_{max} \gets \min(d_{\max\_init}, |s|/2)$   %\texttt{UpdateNeighbor}($d$) %$d \gets d_{min}$
...
A solution $s'$ belongs to the neighborhood structure  $\mathcal{N}_{d}(s)$, $d=d_{\min}, \ldots,  d_{\max}$  around solution $s$ if it can be obtained from $s$ by first removing some $\min(d, |s|)$ vertices from $s$ and then adding $d$ vertices from $V$ into $s$. 
  ...
  \begin{itemize}
  	...
  	$d_{max}$ is dynamically adjusted to $\min(d_{\max\_init}, |s|/2)$ if $|s|\geq4$; otherwise set to $d_{\max\_init}$. The purpose of variable $d_{max\_init}$ is storing the initial value of the variable $d_{max}$.
