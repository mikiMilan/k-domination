...
\subsection{Parameters tuning}
   ...
   Concerning tuning tool, we decided to use an evolutionary--based tool \texttt{PyGAD}. This tool is given as an open-source Python library and can be imported directly by installing the respective package. All (20) small-sized problem instances are separately tuned and thus, for each of them a single parameter setting of VNS was obtained. 
   \fxnote{Milan: dio oko podesavanja tuning alata -- koje instance, koliki je budet broja pustanja, isl.}
  
    
     Details on the configuration settings per each small-sized instance produced by \texttt{PyGAD} tool can be found at \url{XXxxxx.com}
     
     We emphasize that tuning of all (five) large-sized instances    by means of \texttt{PyGAD} tool has not been efficient. Thus, we determined a reasonable  value of the parameters  greedily, by testing a few carefully chosen parameters configurations. Among them, the best performing configuration w.r.t. average solution quality has the following values: $d_{min}=1, d_{max} = + \infty, penalty = 0.01, prob=0.5$. 
