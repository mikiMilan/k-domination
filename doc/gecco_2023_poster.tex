 ...
 The return value of VNS is called $D_{best}$ -- it is the incumbent solution. 
     ...
     \begin{algorithm}[!t] 
  	...
  	\begin{algorithmic}[1]
  		...
  		\STATE  $D_{best} \gets \texttt{LocalSearch}(\emptyset)$ 
  	    ...
  	    \STATE $d \gets d_{\min}$
  	 	\STATE  $d_{max} \gets \min(d_{\max\_init}, |D_{best}|/2)$  
  		\ELSE 
...
\subsection{Parameters tuning}
   ...
   For parameter tuning purposes, the vhile loop in line 5 of the VNS algorithm is limited to 100 iterations. \hl{Maybe some explanation here why it was done that way}.The following genetic algorithm parameters are set:
\begin{itemize}
	\item $num\_generations = 50$ (number of generations),
	\item $num\_parents\_mating = 5$ (number of solutions to be selected as parents in the mating pool),
	\item $sol\_per\_pop = 10$ (number of solutions in the population),
	\item $mutation\_probability = 0.1$  (the probability of selecting a gene for applying the mutation operation),
	\item $num\_genes = 4$ (number of genes in the solution/chromosome).
\end{itemize}
