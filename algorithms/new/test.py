...
class VNS:
    def __init__(self, instance_name, graph: DiGraph or Graph, k: int, d_min: int, d_max_init: int, time_limit: int, iteration_max: int, prob: float, penalty: float, rseed : int):
        ...
        self.neigh = {}
        for v in self.graph.nodes:
            sus = list(self.graph[v])
            self.neigh[v] = (sus, len(sus))
            ...
            for v in self.nodes:
                if v not in s:
                    ...
                    if self.first_fitness_better(new_fit, curr_fit):
                        cache_rec_add(s, v, curr_fit, self.graph, self.neighbors, self.neighb_matrix, self.k, cache)
                        s.add(v)
                        # print("Add ", v)
                        curr_fit = new_fit
                        improved = True
                    ...
                    new_fit = fitness_rec_rem(s, v, curr_fit, self.graph, self.neighbors, self.neighb_matrix, self.k, cache)
                    if self.first_fitness_better(new_fit, curr_fit):
                        cache_rec_rem(s, v, curr_fit, self.graph, self.neighbors, self.neighb_matrix, self.k, cache)
                        s.remove(v)
                        # print("Remove ", v)
                        curr_fit = new_fit
                        improved = True
    ...
    def run(self) -> list:
        ...
        while iteration < self.iteration_max and time()-start_time < self.time_limit:
            ...
            iteration += 1
            # if iteration%100== 0:
            #     print("it={:4d}\tt={:2d}\td={:2d}\tdmin={}\tdmax={}\tprob={:.2f}\tpen={:.4f}\tbest={}\tnew={}\tk={}\tinst={}\tfit={}".format(iteration, int(time() - start_time),d,self.d_min, self.d_max, self.prob, self.penalty, fit, fit_new, self.k, self.instance_name, (fit[0]+1)*(1+fit[1]*self.penalty)))
            
        tot_time = time()-start_time
        
        return s_accept, best_time, fit[0]==0, ((fit[0]+1)*(1+fit[1]*self.penalty))
...
if __name__ == '__main__':
    ...
    file_name_res = 'results/test/k' + str(k) +'_tl'+str(time_limit)+'_it'+str(iteration_max)+'_seed'+str(rseed)+'.txt'
