...
def fitness_rec_add_cache_change(s: set, v: int, fit: tuple, g: Graph or DiGraph, all_neighbors: dict, neighb_matrix: list, k: int, cache) -> float:
    viol = fit[0]
    size = fit[1]+1
    ineff = fit[2]
    max_ineff = fit[3]
...
def fitness_rec_rem_cache_change(s: set, v: int, fit: tuple, g: Graph or DiGraph, all_neighbors: dict, neighb_matrix: list, k: int, cache: dict) -> float:
